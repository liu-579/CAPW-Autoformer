...
class ModelConfig:
    ...
    d_model = 128  # 隐藏层维度 (Embedding维度)
    nhead = 8  # 多头注意力头数 (必须能被 d_model 整除)
    num_layers = 3  # Transformer Encoder 层数
    dim_feedforward = 512  # 前馈网络隐藏层维度 (通常是 d_model 的 4 倍)
    ...
    # ==================== 输出层参数 ====================
    seq_len = 720   # 输入序列长度（使用过去30天数据）
    output_dim = 1  # 输出维度 (单变量预测)
    pred_len = 168  # 预测长度 (未来7天)
