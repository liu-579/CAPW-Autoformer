...
"""
M9 BP神经网络 模型配置文件
BP (Backpropagation) Neural Network Baseline Configuration
用途: 定义 BP 神经网络基线模型的超参数、架构配置
说明: 作为基线模型，将主干网络从 Autoformer/Transformer 替换为多层全连接网络 (MLP)
...
class ModelConfig:
    """
    BP 神经网络动态融合预测器配置类
    主干: BP Neural Network (Multi-Layer Perceptron)
    ...
    # MLP 隐藏层维度列表
    # 例如 [512, 256, 128] 表示三层隐藏层，维度依次为 512→256→128
    ...
    # 是否使用残差连接 (当相邻隐藏层维度相同时启用)
    use_residual = False

    # ==================== 权重网络参数 ====================

    # Context-Aware Weight Network 参数 (与 Autoformer 版保持一致)
    ...
    save_attention_weights = False  # BP 无注意力机制
    ...
    def from_dataset(cls, dataset_dir: str, verbose: bool = True):
        ...
        if verbose:
            print(f"\n{'=' * 70}")
            print("ModelConfig (BP Neural Network): 从数据集自动加载参数")
            print(f"{'=' * 70}")
    ...
    def validate(cls):
        """验证配置参数的合法性"""
        assert len(cls.hidden_dims) >= 1, \
            f"hidden_dims 至少需要一层，当前: {cls.hidden_dims}"

        assert all(d > 0 for d in cls.hidden_dims), \
            f"hidden_dims 中所有维度必须大于 0，当前: {cls.hidden_dims}"
        ...
        assert cls.activation in ['relu', 'gelu', 'silu', 'tanh'], \
        ...
        print("✓ 配置参数验证通过 (BP Neural Network)")
        return True
    ...
    def print_config(cls):
        """打印配置信息"""
        print("\n" + "=" * 70)
        print("Model Configuration (BP Neural Network)")
        print("=" * 70)
        ...
        print("\n【BP Neural Network Backbone】")
        print(f"  - hidden_dims:      {cls.hidden_dims}")
        print(f"  - dropout:          {cls.dropout}")
        print(f"  - use_batch_norm:   {cls.use_batch_norm}")
        print(f"  - use_residual:     {cls.use_residual}")
...
if __name__ == "__main__":
    print("\n" + "#" * 70)
    print("# ModelConfig (BP Neural Network) 测试")
    print("#" * 70)
