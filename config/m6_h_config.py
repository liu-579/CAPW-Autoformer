...
SOURCE_FIELDS = {
    ...
    'date': 'date',                  # 日期字段
    'hour': 'hourNumber',            # 小时字段（0-23，天粒度数据为null）
...
# 小时格式
HOUR_FORMAT = '%H'  # 小时格式 "00" 到 "23"


# ==================== 缺失数据填充配置 ====================
# 缺失数据填充方法
FILL_METHOD = 'interpolate'  # 可选: 'interpolate'(线性插值), 'forward'(前向填充), 'backward'(后向填充)

# 插值窗口大小（用于前后均值填充）
INTERPOLATE_WINDOW = 3  # 前后各取3个小时的数据

# 是否允许边界外推
ALLOW_EXTRAPOLATION = True  # 对于首尾缺失值，是否使用最近的有效值填充
...
# ==================== 输出配置 ====================
# 输出表名模板
OUTPUT_TABLE_TEMPLATE = 'history_hourly_{fence_id}'
...
# 是否在发现数据异常时打印警告
WARN_ON_MISSING_DATA = True  # 当发现缺失数据时警告
WARN_ON_MISSING_WEATHER = True  # 当某小时有客流但无天气数据时警告
...
MIN_RECORDS_THRESHOLD = 100  # 小时粒度数据量更大

# 是否检查时间连续性
CHECK_TIME_CONTINUITY = True

# 允许的最大连续缺失小时数
MAX_CONSECUTIVE_MISSING_HOURS = 6  # 超过6小时连续缺失则警告