...
"""
M8B 张量构建主程序 (小时级数据粒度)
Data Pipeline Engineer: Leakage-Free Tensor Builder (Hourly)
...
# 导入配置文件（小时级）
import config.m8b_config_h as config

warnings.filterwarnings('ignore')


class TensorBuilder:
    """张量构建器 - 实现防泄露的时序数据管道 (混合归一化策略版 - 小时级)"""
    ...
    def run(self):
        """执行流程"""
        print(f"\n{'#' * 70}")
        print(f"# M8B 张量构建 (Hybrid Normalization - 小时级)")
        print(f"{'#' * 70}")
