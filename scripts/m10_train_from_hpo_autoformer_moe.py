# -*- coding: utf-8 -*-
"""
M10 Autoformer + MoE 从 HPO 结果中加载最优超参，执行全量训练脚本
用途:
  ...
  - 调用 m10_train_moe.main() 执行全量训练

与 m10_train_from_hpo_autoformer.py 的区别:
  - 导入 MoE 版训练入口 (m10_train_moe.main)
  - 默认路径指向 MoE HPO 输出目录
  - 输出目录和 RUN_NAME 带 moe 前缀

使用方法:
  1. 修改下方 ===================== 手动配置区 ===================== 中的路径
  2. 直接运行本脚本：python scripts/m10_train_from_hpo_autoformer_moe.py
...
#   2. 某个 trial_NNN 目录路径      → 读取该 trial 的参数
...
if a == 1:
    TRIAL_SOURCE = (
        PROJECT_ROOT
        / 'data/output/hpo/moe/m10_autoformer_moe_optuna_1_eedscy_20260405_005101/optuna_summary_running.csv'
    )
    # --- [2] 指定要从 summary.csv 中选取哪个 trial ---
    SELECT_TRIAL_INDEX = 19

if a == 2:
    TRIAL_SOURCE = (
        PROJECT_ROOT
        / 'data/output/hpo/moe/m10_autoformer_moe_optuna_1_eedscy_YYYYMMDD_HHMMSS/best_trial.json'
    )
...
OUTPUT_SAVE_DIR = 'data/output/h_out_eedscy/m10_moe_trial19_full_new'   # 例如: 'data/output/h_out_eedscy/m10_moe_trial000_full'
...
def build_full_train_overrides(
    ...
    if output_save_dir:
        ...
        save_dir = (
            PROJECT_ROOT
            / f'data/output/h_out_eedscy/m10_autoformer_moe_full_trial{trial_index:03d}_{timestamp}'
        )
    save_dir.mkdir(parents=True, exist_ok=True)
    training['SAVE_DIR'] = save_dir
    training['RUN_NAME'] = f'autoformer_moe_full_trial_{trial_index:03d}'
...
def print_summary(loaded: dict, training: dict, model: dict):
    """打印本次全量训练配置摘要"""
    print('\n' + '=' * 70)
    print('M10 Autoformer + MoE 全量训练（从 HPO 结果加载超参）')
    print('=' * 70)
