# -*- coding: utf-8 -*-
"""
M10 BP神经网络 基线训练脚本 (CCC Loss Version + Adaptive Peak Weighting)
BP Neural Network Baseline Training Pipeline
用途: 训练 BP 神经网络基线模型，用于与 Autoformer/Transformer 等复杂模型进行对比实验。
      训练策略与 m10_train_2.py 完全一致 (CCC Loss + 自适应峰值加权)，
      仅将模型和配置替换为 BP 版本，确保对比实验的公平性。
修改点 (相对于 m10_train_2.py):
1. 导入 m9_model_bp.build_model 替代 m9_model_2.build_model
2. 导入 config.m9_config_bp.ModelConfig 替代 config.m9_config.ModelConfig
3. SAVE_DIR 修改为 BP 专用目录
4. 其余训练逻辑、Loss 函数、早停、评估等完全不变
"""
...
# ==================== 【核心修改】导入 BP 模型和配置 ====================
sys.path.append('.')
from m9_model_bp import build_model                     # BP 模型
from config.m9_config_bp import ModelConfig              # BP 模型配置
from config.m10_config import TrainingConfig as cfg      # 训练配置 (复用)


# ==================== 【修改】覆盖保存目录为 BP 专用 ====================
# 避免覆盖 Autoformer/Transformer 的训练结果
cfg.SAVE_DIR = Path(cfg.BASE_DIR / f'data/output/h_out_{cfg.scenic_name}/m10_checkpoints_{cfg.scenic_name}_bp_h_4d')
...
class UnifiedTrainer:
    """
    统一联合训练器 (BP Neural Network Baseline)
    ...
    def __init__(self, model, train_loader, val_loader, test_loader,
        ...
        print(f"🧠 Backbone: BP Neural Network (MLP Baseline)")
    ...
    def train(self):
        print(f"\n{'=' * 70}")
        print("开始联合训练 (BP Neural Network Baseline + CCC + Adaptive Weighted MSE)")
        print(f"策略: End-to-End Training | Main Loss: CCC | Backbone: BP (MLP)")
        if getattr(self.config, 'USE_ADAPTIVE_WEIGHT', False):
    ...
    def evaluate_final_model(self):
        ...
        print(f"\n🔎 正在评估最佳模型 (BP Baseline): {model_path}")
        ...
        print(f"   📈 测试集最终结果 (Real Space) [BP Neural Network Baseline]:")
        ...
        self.plot_results(preds, targets, 'Final_BP_Baseline', test_metrics['RMSE'])
    ...
    def plot_training_curves(self):
        ...
        fig.suptitle('Training Curves (BP Neural Network Baseline)', fontsize=16, fontweight='bold')
...
def main(training_overrides=None, model_overrides=None, reset_model_config=True,
         ...
         prune_enabled=False):
    print(f"\n{'=' * 70}\nM10 BP Neural Network 基线训练 (Adaptive Peak Weighting)\n{'=' * 70}")
