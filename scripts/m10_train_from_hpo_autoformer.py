# -*- coding: utf-8 -*-
"""
M10 Autoformer 从 HPO 结果中加载最优超参，执行全量训练脚本
用途:
  ...
  - 调用 m10_train_autoformer.main() 执行全量训练

与 m10_train_from_hpo.py 的区别:
  - 导入 Autoformer 版训练入口 (m10_train_autoformer.main)
  - 默认路径指向 Autoformer HPO 输出目录
  - 输出目录和 RUN_NAME 带 autoformer 前缀

使用方法:
  1. 修改下方 ===================== 手动配置区 ===================== 中的路径
  2. 直接运行本脚本：python scripts/m10_train_from_hpo_autoformer.py
...
#   3. optuna_summary.csv 或 optuna_summary_running.csv 文件路径
# 示例（切换 trial 只改这一行）：
a=1
if a==1:
    TRIAL_SOURCE = (
        PROJECT_ROOT
        / 'data/output_xsw/hpo/auto2_ablation_no_sentiment/m10_autoformer_optuna_ablation_no_sentiment_xsw_20260414_203058/optuna_summary_running.csv'
    )

    # --- [2] 指定要从 summary.csv 中选取哪个 trial（仅当 TRIAL_SOURCE 指向 csv 时有效）---
    #   填写 trial 编号（整数），例如 27 表示 trial_027（当前最佳 best_val_ccc_loss=0.1238）
    SELECT_TRIAL_INDEX = 33

if a==2:
    TRIAL_SOURCE = (
        PROJECT_ROOT
        / 'data/output/hpo/m10_autoformer_optuna_eedscy_20260312_123456/best_trial.json'
    )
    SELECT_TRIAL_INDEX = 0  # 使用 best_trial.json 时此项无效

# --- [3] 全量训练覆盖参数（在 HPO 快速模式基础上恢复）---
FULL_TRAIN_OVERRIDES = {
    'NUM_EPOCHS':    1000,    # 恢复完整轮次（HPO 快速模式为 150）
    'PATIENCE':      30,      # 恢复完整早停（HPO 快速模式为 20）
    'SAVE_INTERVAL': 5,       # 恢复正常保存间隔（HPO 快速模式为 999999）
}

# --- [4] 输出目录（留空则自动生成带时间戳的目录）---
OUTPUT_SAVE_DIR = ''   # 例如: 'data/output/h_out_eedscy/m10_autoformer_trial027_full'
...
def _load_from_summary_csv(path: Path, trial_index: int) -> dict:
    """从 optuna_summary.csv 或 optuna_summary_running.csv 读取指定 trial 的参数"""
    import pandas as pd
...
def load_trial_params(source: Path, select_trial_index: int) -> dict:
    ...
    if source.is_dir():
        ...
        candidate_running = source.parent / 'optuna_summary_running.csv'
        # 从目录名提取 trial 编号
        dir_name = source.name  # 例如 trial_027
        ...
        elif candidate_running.exists():
            print(f'📂 检测到 trial 目录，自动从同级 optuna_summary_running.csv 加载 trial_{idx:03d}')
            return _load_from_summary_csv(candidate_running, idx)
...
def build_full_train_overrides(
    ...
    if output_save_dir:
        ...
        save_dir = (
            PROJECT_ROOT
            / f'data/output_xsw/h_out_xsw/m10_autoformer_full_trial{trial_index:03d}_{timestamp}'
        )
    save_dir.mkdir(parents=True, exist_ok=True)
    training['SAVE_DIR'] = save_dir
    training['RUN_NAME'] = f'autoformer_full_trial_{trial_index:03d}'
...
def print_summary(loaded: dict, training: dict, model: dict):
    """打印本次全量训练配置摘要"""
    print('\n' + '=' * 70)
    print('M10 Autoformer 全量训练（从 HPO 结果加载超参）')
    print('=' * 70)
